(function () {
  ...
  const boardStorageKey = "infinite-paper:v8.5:board";
  const viewStorageKey = "infinite-paper:v8.5:view";
  const syncChannelName = "infinite-paper:v8.5:sync";
  const panThreshold = 5;
  const guideDistance = 8;
  ...
  function updateAlignmentGuides(dx, dy) {
    ...
    let guideWorldX = null;
    let guideWorldY = null;
    let bestX = guideDistance + 1;
    let bestY = guideDistance + 1;
    ...
    for (const rect of getStationaryNoteRects()) {
      for (const targetX of [rect.left, rect.centerX, rect.right]) {
        for (const sourceX of movingX) {
          ...
          if (distance < bestX && distance <= guideDistance) {
            bestX = distance;
            guideWorldX = targetX;
          }
        }
      ...
      for (const targetY of [rect.top, rect.centerY, rect.bottom]) {
        for (const sourceY of movingY) {
          ...
          if (distance < bestY && distance <= guideDistance) {
            bestY = distance;
            guideWorldY = targetY;
          }
        }
    ...
    if (guideWorldX === null) {
      ...
      guideX.hidden = false;
      guideX.style.transform = `translateX(${view.x + guideWorldX * view.scale}px)`;
    }
    ...
    if (guideWorldY === null) {
      ...
      guideY.hidden = false;
      guideY.style.transform = `translateY(${view.y + guideWorldY * view.scale}px)`;
    }
  }
