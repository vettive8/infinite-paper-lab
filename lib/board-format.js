...
/** Same mapping the app and the attachments endpoint use for filenames. */
...
export function serializeBoard(board) {
  ...
  const lines = [
    ...
    `folder: ${yamlString(board.folder || "")}`,
  ...
  for (const note of board.notes || []) {
    if (note.type === "image") {
      ...
      // The body is a real markdown image link (relative to boards/), so
      // the file previews correctly in VS Code / GitHub / Obsidian. The
      // app itself resolves the image from imageId; the body is derived
      // and regenerated on every save.
      lines.push(
        note.imageId
          ? `![](../attachments/${note.imageId}.${mimeToExt(note.mimeType)})`
          : ""
      );
      lines.push("<!-- /ip-note -->", "");
    } else {
  ...
  if (fmMatch) {
    ...
    for (const raw of fmMatch[1].split("\n")) {
      ...
      if (key === "view") {
        ...
        board.folder = parseYamlString(value);
      } else if (
