...
- Press `F2` to rename the current browser tab. The name is saved for that tab and survives refresh.
...
- Press `M` to mirror selected images horizontally, or `Shift` + `M` to mirror them vertically. Text does not mirror.
