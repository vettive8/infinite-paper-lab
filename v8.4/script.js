(function () {
  ...
  const boardStorageKey = "infinite-paper:v8.4:board";
  const viewStorageKey = "infinite-paper:v8.4:view";
  const syncChannelName = "infinite-paper:v8.4:sync";
  ...
  function getSelectedNoteMeasurements() {
    return notes
      .filter((note) => selectedNoteIds.has(note.id))
      .map((note) => {
        const element = paper.querySelector(`[data-id="${CSS.escape(note.id)}"]`);
  ...
  function alignSelectedNotes(mode) {
    const selected = getSelectedNoteMeasurements();
    if (selected.length < 2) {
      return false;
    }

    syncNotesFromDom();
    const left = Math.min(...selected.map((item) => item.note.x));
    const top = Math.min(...selected.map((item) => item.note.y));
    const right = Math.max(...selected.map((item) => item.note.x + item.width));
    const bottom = Math.max(...selected.map((item) => item.note.y + item.height));
    ...
    for (const item of selected) {
      ...
      if (mode === "right") {
        item.note.x = right - item.width;
      }
      if (mode === "top") {
        item.note.y = top;
      }
      if (mode === "bottom") {
        item.note.y = bottom - item.height;
      }

      item.element.style.left = `${item.note.x}px`;
      item.element.style.top = `${item.note.y}px`;
    }
  ...
  window.addEventListener("keydown", (event) => {
    ...
    if (selectedNoteIds.size > 1 && event.ctrlKey && event.altKey) {
      const alignments = {
        ArrowLeft: "left",
        ArrowRight: "right",
        ArrowUp: "top",
      ...
      const mode = alignments[event.key];
      if (mode && alignSelectedNotes(mode)) {
        event.preventDefault();
        return;
      }
    }
