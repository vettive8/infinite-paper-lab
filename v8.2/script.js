(function () {
  ...
  const boardStorageKey = "infinite-paper:v8.2:board";
  const viewStorageKey = "infinite-paper:v8.2:view";
  const syncChannelName = "infinite-paper:v8.2:sync";
      ...
      startNotes: notes
        ...
        .map((note) => {
          ...
          return {
            ...
            x: note.x,
            y: note.y,
            lineTops: getNoteLineTops(note, element),
          };
        }),
    };
    viewport.setPointerCapture(event.pointerId);
  ...
  function getNoteLineTops(note, element) {
    if (!element) {
      return [note.y];
    }
    ...
    const lineCount = Math.max(1, (note.text || "").split("\n").length);
    return Array.from({ length: lineCount }, (_, index) => note.y + paddingTop + index * lineHeight);
  }

  function getStationaryLineTops() {
    return notes
      .filter((note) => !selectedNoteIds.has(note.id))
      .flatMap((note) => {
        const element = paper.querySelector(`[data-id="${CSS.escape(note.id)}"]`);
        return getNoteLineTops(note, element);
      });
  }

  function getTextLineSnapOffset(dy) {
    const sourceLineTops = activeDrag.startNotes.flatMap((note) =>
      note.lineTops.map((lineTop) => lineTop + dy)
    );
    const targetLineTops = getStationaryLineTops();
    let snapY = 0;
    let bestY = snapDistance + 1;

    for (const sourceY of sourceLineTops) {
      for (const targetY of targetLineTops) {
